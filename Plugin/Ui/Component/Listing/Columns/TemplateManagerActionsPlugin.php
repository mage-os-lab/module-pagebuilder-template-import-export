...
class TemplateManagerActionsPlugin
{
     ...
     *
     * @param \Magento\PageBuilder\Ui\Component\Listing\Columns\TemplateManagerActions $subject
     * @param array $result
     * @return array
     */
    public function afterPrepareDataSource(
        \Magento\PageBuilder\Ui\Component\Listing\Columns\TemplateManagerActions $subject,
        array $result
    ) {
        if (isset($result['data']['items'])) {
            foreach ($result['data']['items'] as &$item) {
                ...
                if (isset($item[$indexField])) {
                    if ($this->authorization->isAllowed('MageOS_PageBuilderTemplateImportExport::pagebuilder_template_export')) {
                        $item[$name]['export'] = [
                            'label' => __('Export'),
